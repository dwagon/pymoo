...
def initial_data(apps, schema_editor):
    ...
    t = TT.objects.get(name='Soil Enrichment')
    BB(name='Soil Enrichment', cost=120, maint=0, required=t, desc="""Widespread use of nano-machinery allows planetary engineers to alter the chemical make-up of the colony’s topsoil. This “fertilisation” process increases the food output of each farming unit of population by 1. For different reasons, Soil Enrichment does not work in hostile climates. Barren worlds have no topsoil to work on, while ongoing chemical processes in the soils of Radiated and Toxic planets undo the fertilisation as fast as it is done.""").save()

    t = TT.objects.get(name='Terraforming')
    BB(name='Terraforming 1', cost=250, maint=0, required=t, desc="""Terraforming is a slow process of altering the environmental characteristics of a planet until they closely resemble Terran norms. This changes weather patterns, stabilises extreme temperature fluctuations, and adjusts the abundance of surface water. Terraforming will only work on planets that have hospitable environments already. Barren worlds become Desert or Tundra, Desert environments become Arid, Tundra planets become Swamp worlds, and Ocean, Arid, and Swamp become Terran. You can terraform a planet several times, but each application has an increased production cost.""").save()
    BB(name='Terraforming 2', cost=500, maint=0, required=t, desc="""Terraforming is a slow process of altering the environmental characteristics of a planet until they closely resemble Terran norms. This changes weather patterns, stabilises extreme temperature fluctuations, and adjusts the abundance of surface water. Terraforming will only work on planets that have hospitable environments already. Barren worlds become Desert or Tundra, Desert environments become Arid, Tundra planets become Swamp worlds, and Ocean, Arid, and Swamp become Terran. You can terraform a planet several times, but each application has an increased production cost.""").save()
    BB(name='Terraforming 3', cost=750, maint=0, required=t, desc="""Terraforming is a slow process of altering the environmental characteristics of a planet until they closely resemble Terran norms. This changes weather patterns, stabilises extreme temperature fluctuations, and adjusts the abundance of surface water. Terraforming will only work on planets that have hospitable environments already. Barren worlds become Desert or Tundra, Desert environments become Arid, Tundra planets become Swamp worlds, and Ocean, Arid, and Swamp become Terran. You can terraform a planet several times, but each application has an increased production cost.""").save()

    t = TT.objects.get(name='Gaia Transformation')
    BB(name='Gaia Transformation', cost=500, maint=0, required=t, desc="""The Gaia Transformation introduces genetically engineered micro-organisms into a world to create an environment perfectly suited to both plant and animal growth. The transformation can be applied to Terran environments. Afterward, the planet becomes a Gaia class world.""").save()
